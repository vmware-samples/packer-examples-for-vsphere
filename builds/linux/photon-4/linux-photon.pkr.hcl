...
source "vsphere-iso" "linux-photon" {
  ...
  http_content     = {
    "/ks.json"       = templatefile("${path.cwd}/data/data/ks.pkrtpl.hcl", { build_username = var.build_username, build_password_encrypted = var.build_password_encrypted })
    "/packages.json" = file("${path.cwd}/data/data/packages_minimal.json")
  }
