...
variable "iso_file" {
  type        = string
  description = "The file name of the ISO image. (e.g. 'iso-linux-ubuntu-server-18-04-lts.iso')"
}

variable "iso_checksum" {
  type        = string
  description = "The checksum of the ISO image. (e.g. Result of 'shasum -a 512 iso-linux-ubuntu-server-18-04-lts.iso')"
}
...
variable "http_directory" {
  type        = string
  description = "The HTTP directory path. (e.g. ../../../configs/linux/ubuntu-server/)"
}
...
source "vsphere-iso" "linux-ubuntu-server" {
  ...
  boot_command = ["<enter><wait><f6><wait><esc><wait>",
    ...
    " url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/${var.http_file}",
  "<enter>"]
