...
source "vsphere-iso" "windows-server-standard-core" {
  ...
  cd_content = {
    "autounattend.xml" = templatefile("${abspath(path.root)}/data/autounattend.pkrtpl.hcl", {
      os_image             = "Windows Server 2022 SERVERSTANDARDCORE"
      kms_key              = "VDYBN-27WPP-V4HQT-9VMD4-VMK7H"
...
source "vsphere-iso" "windows-server-standard-dexp" {
  ...
  cd_content = {
    "autounattend.xml" = templatefile("${abspath(path.root)}/data/autounattend.pkrtpl.hcl", {
      os_image             = "Windows Server 2022 SERVERSTANDARD"
      kms_key              = "VDYBN-27WPP-V4HQT-9VMD4-VMK7H"
      build_username       = var.build_username
      build_password       = var.build_password
      vm_guest_os_language = var.vm_guest_os_language
...
source "vsphere-iso" "windows-server-datacenter-core" {
  ...
  cd_content = {
    "autounattend.xml" = templatefile("${abspath(path.root)}/data/autounattend.pkrtpl.hcl", {
      os_image             = "Windows Server 2022 SERVERDATACENTERCORE"
